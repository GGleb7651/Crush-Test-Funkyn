...
class MenuLua {
	...
	public function new(script:String) {
		...
		/*Lua_helper.add_callback(lua, "keyJustPressed", function(name:String) {
			var key:Bool = false;
			switch(name) {
				case 'left': key = MainMenuState.instance.getControl('NOTE_LEFT_P');
				case 'down': key = MainMenuState.instance.getControl('NOTE_DOWN_P');
				case 'up': key = MainMenuState.instance.getControl('NOTE_UP_P');
				case 'right': key = MainMenuState.instance.getControl('NOTE_RIGHT_P');
				case 'accept': key = MainMenuState.instance.getControl('ACCEPT');
				case 'back': key = MainMenuState.instance.getControl('BACK');
				case 'pause': key = MainMenuState.instance.getControl('PAUSE');
				case 'reset': key = MainMenuState.instance.getControl('RESET');
				case 'space': key = FlxG.keys.justPressed.SPACE;//an extra key for convinience
			}
			return key;
		});
		Lua_helper.add_callback(lua, "keyPressed", function(name:String) {
			var key:Bool = false;
			switch(name) {
				case 'left': key = MainMenuState.instance.getControl('NOTE_LEFT');
				case 'down': key = MainMenuState.instance.getControl('NOTE_DOWN');
				case 'up': key = MainMenuState.instance.getControl('NOTE_UP');
				case 'right': key = MainMenuState.instance.getControl('NOTE_RIGHT');
				case 'space': key = FlxG.keys.pressed.SPACE;//an extra key for convinience
			}
			return key;
		});
		Lua_helper.add_callback(lua, "keyReleased", function(name:String) {
			var key:Bool = false;
			switch(name) {
				case 'left': key = MainMenuState.instance.getControl('NOTE_LEFT_R');
				case 'down': key = MainMenuState.instance.getControl('NOTE_DOWN_R');
				case 'up': key = MainMenuState.instance.getControl('NOTE_UP_R');
				case 'right': key = MainMenuState.instance.getControl('NOTE_RIGHT_R');
				case 'space': key = FlxG.keys.justReleased.SPACE;//an extra key for convinience
			}
			return key;
		});*/
		...
		Lua_helper.add_callback(lua, "addCameraZoom", function(cam:String,addZoomShit:Float) {
			MainMenuState.instance.addCamZoom(cam,addZoomShit);
		});
		...
		Lua_helper.add_callback(lua, "addLuaSprite", function(tag:String, front:Bool = false) {
			if(MainMenuState.instance.modchartSprites.exists(tag)) {
				var shit:ModchartSprite = MainMenuState.instance.modchartSprites.get(tag);
				if(!shit.wasAdded) {
					if(front)
					{
						getInstance().add(shit);
					...
					else
					{
						...
						var position:Int = 1;
						MainMenuState.instance.insert(position, shit);
		...
		Lua_helper.add_callback(lua, "removeLuaSprite", function(tag:String, destroy:Bool = true) {
			...
			var pee:ModchartSprite = MainMenuState.instance.modchartSprites.get(tag);
			if(destroy) {
				pee.kill();
			}
		...
		Lua_helper.add_callback(lua, "addLuaText", function(tag:String) {
			if(MainMenuState.instance.modchartTexts.exists(tag)) {
				var shit:ModchartText = MainMenuState.instance.modchartTexts.get(tag);
				if(!shit.wasAdded) {
					getInstance().add(shit);
					shit.wasAdded = true;
					//trace('added a thing: ' + tag);
		...
		Lua_helper.add_callback(lua, "removeLuaText", function(tag:String, destroy:Bool = true) {
			...
			var pee:ModchartText = MainMenuState.instance.modchartTexts.get(tag);
			if(destroy) {
				pee.kill();
			}
		...
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByPrefix", function(tag:String, name:String, prefix:String, framerate:Int = 24, loop:Bool = true) {
			luaTrace("luaSpriteAddAnimationByPrefix is deprecated! Use addAnimationByPrefix instead", false, true);
			if(MainMenuState.instance.modchartSprites.exists(tag)) {
				var cock:ModchartSprite = MainMenuState.instance.modchartSprites.get(tag);
				cock.animation.addByPrefix(name, prefix, framerate, loop);
				if(cock.animation.curAnim == null) {
					cock.animation.play(name, true);
				}
		...
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByIndices", function(tag:String, name:String, prefix:String, indices:String, framerate:Int = 24) {
			...
			if(MainMenuState.instance.modchartSprites.exists(tag)) {
				...
				var pussy:ModchartSprite = MainMenuState.instance.modchartSprites.get(tag);
				pussy.animation.addByIndices(name, prefix, die, '', framerate, false);
				if(pussy.animation.curAnim == null) {
					pussy.animation.play(name, true);
				}
